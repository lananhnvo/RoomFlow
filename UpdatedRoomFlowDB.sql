...
('41 Meteor Meadow', 'Celestia', 'Dreamscape Delta', '41mm@celestia.com', '+75-882-849-7611', 1, 'economy', 5, NULL),
...
UPDATE Hotel SET manager=848484 WHERE address='84 Starlight Street';
...
-- Search for number of hotels that each chain has
-- AGGREGATION
SELECT HotelChain.chain, COUNT(Hotel.address) AS num_of_hotels, HotelChain.central_office_address, HotelChain.email, HotelChain.phone_num
FROM HotelChain
JOIN Hotel ON HotelChain.chain = Hotel.chain
GROUP BY HotelChain.chain;

-- Search for room based on chain, capacity, price and check for availability if already booked
-- NESTED
SELECT Room.room_num, Hotel.address, Room.capacity, Room.price, Hotel.area, HotelChain.chain, Hotel.category
FROM Room
JOIN Hotel ON Room.hotel = Hotel.address
JOIN HotelChain ON Hotel.chain = HotelChain.chain
WHERE Hotel.address = '41 Meteor Meadow'
AND Hotel.category = 'economy'
AND Room.capacity >= 2
AND Room.price BETWEEN 200 AND 250
AND Room.room_num NOT IN (
SELECT room_num
      		FROM Booking
      		WHERE (start_date <= '2020-01-01' AND end_date >= '2020-01-30')
  )
ORDER BY Room.price ASC;


-- Convert Booking into a Renting
SELECT * FROM Booking;
INSERT INTO Customer (sin, full_name, address, registration_date)
VALUES (651970, 'Lily Tran', '88 Rideau Street', '2024-03-31');
INSERT INTO Booking(room_num, hotel, customer, employee, start_date, end_date, ispaid) 
VALUES (1, '41 Meteor Meadow', 651970, NULL, '2024-04-01', '2024-04-04', false);
	-- demo
UPDATE Booking
SET employee = 414141, isPaid = true
WHERE customer = 651970;

SELECT * FROM Booking;

-- Search for Employee Info Based on Hotel
SELECT Employee.SIN, Employee.full_name, Employee.address, Employee.position, Employee.works_at
FROM Employee
WHERE Employee.works_at = '73 Phoenix Avenue';
...
-- TRIGGER 1: Archive Booking before deleting
...
-- inserted
INSERT INTO Customer (sin, full_name, address, registration_date)
VALUES (123456, 'Sami Sami', '32 Elgin Street', '2024-02-28');
INSERT INTO Booking (room_num, hotel, customer, employee, start_date, end_date, ispaid) 
VALUES (5, '43 Meteor Meadow', 123456, NULL, '2024-03-01', '2024-03-02', false);

CREATE OR REPLACE FUNCTION archive_booking() RETURNS TRIGGER AS $$
BEGIN
    INSERT INTO Archive (archived_data)
    VALUES (
        'Start Date: ' || OLD.start_date || ', End Date: ' || OLD.end_date || 
        ', Customer: ' || OLD.customer || ', Is Paid: ' || OLD.isPaid || 
        ', Employee: ' || COALESCE(CAST((SELECT E.SIN FROM Employee E WHERE E.SIN = OLD.employee) AS TEXT), 'None')
    );
    RETURN OLD;
END;
$$ LANGUAGE plpgsql;
...
-- demo
DELETE FROM Booking
WHERE room_num = '5' AND hotel = '43 Meteor Meadow';


-- TRIGGER 2: Ensure that a Hotel has an associated manager

CREATE OR REPLACE FUNCTION fn_check_manager_constraint_upd()
RETURNS TRIGGER AS $$
DECLARE
    manager_count INT;
BEGIN
    -- Check if the updated employee is a manager being demoted
    IF OLD.position = 'manager' AND NEW.position != 'manager' THEN
        -- Count remaining managers in the hotel
        SELECT COUNT(*) INTO manager_count
        FROM Employee
        WHERE works_at = OLD.works_at AND position = 'manager' AND works_at != OLD.works_at;

        IF manager_count = 0 THEN
            RAISE EXCEPTION 'Violation of constraint: Each hotel must have at least one manager. Hotel ID % has no remaining manager after update.', OLD.works_at;
        END IF;
    END IF;
...
FOR EACH ROW
EXECUTE FUNCTION fn_check_manager_constraint_upd();

SELECT * FROM Employee;

-- demo
UPDATE Employee SET position = 'receptionist' WHERE works_at = '41 Meteor Meadow';



-- 7) INDEXES
CREATE INDEX room_status_index on Room (status);
	-- count total number of rooms by status
CREATE INDEX hotel_area_index on Hotel (area);
	-- count total number of hotels in a certain area
CREATE INDEX room_capacity_index on Room (capacity);
	-- calculate total capacity of rooms
